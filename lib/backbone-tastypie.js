...
(function($, _, Backbone) {
    ...
    Backbone.Tastypie.addAuthentication = function(method, model, options) {
        ...
        if (method == "read") {
            ...
            options.url += "?username=" + Backbone.Tastypie.authCredentials.username + "&api_key=" + Backbone.Tastypie.authCredentials.apiKey;
        }
    ...
    Backbone.Tastypie.Model = Backbone.Model.extend({
        ...
        destroy: function(options) {
            ...
            options.success = _.bind(function(model, resp, options) {
                this.trigger("sync:destroy");
                if (success) {
